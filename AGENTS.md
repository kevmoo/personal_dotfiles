...
- **Literal Unicode Emojis (No Shortcodes)**: Always emit native Unicode emoji characters (e.g., `✅`, `❌`, `⚠️`, `🎉`, `🚀`, `🔥`, `⏳`) rather than GitHub-style markdown shortcodes (e.g., `:white_check_mark:`, `:heavy_check_mark:`, `:warning:`, `:tada:`). The chat UI and g3doc markdown engines render standard CommonMark without shortcode expansion, displaying `:shortcode:` as unrendered literal text. When quoting or summarizing GitHub CLI (`gh`) tool outputs containing shortcodes, translate them to Unicode emojis or clean text.
