...
''');
    ...
    test('check only inspects the newest git SHA directory when multiple exist', () async {
      Directory(
        '${tempInstallDir.path}/app-bundles/kevmoo_scripts/git/1111111111111111111111111111111111111111',
      ).createSync(recursive: true);

      final newGitDir = Directory(
        '${tempInstallDir.path}/app-bundles/kevmoo_scripts/git/2222222222222222222222222222222222222222',
      )..createSync(recursive: true);

      final pubspecLock = File('${newGitDir.path}/pubspec.lock');
      pubspecLock.writeAsStringSync('''
packages:
  kevmoo_scripts:
    description:
...
''');
      ...
      final status = await upkeeper.check();
      check(status.state).equals(UpkeepState.upToDate);
      check(status.summary).equals('1 tool(s) up to date (kevmoo_scripts)');
    });

    test('check detects outdated local path packages', () async {
      final localDir = Directory(
        '${tempInstallDir.path}/app-bundles/my_tool/local',
      )..createSync(recursive: true);
      Directory('${localDir.path}/bundle').createSync(recursive: true);

      final sourceRepo = Directory('${tempInstallDir.path}/source_repo')
        ..createSync(recursive: true);

      final pubspecLock = File('${localDir.path}/pubspec.lock');
      pubspecLock.writeAsStringSync('''
packages:
  my_tool:
    description:
...
''');
      ...
      final upkeeper = DartInstallUpkeeper(
        installDirOverride: tempInstallDir,
        processRunner: (executable, args) async {
          if (executable == 'git' && args.contains('log')) {
            // Return a commit epoch in the future (newer than bundle mtime)
            final futureEpoch =
                DateTime.now()
                    .add(const Duration(hours: 1))
                    .millisecondsSinceEpoch ~/
                1000;
            return ProcessResult(0, 0, '$futureEpoch abcdef1\n', '');
          }
          return ProcessResult(0, 0, '', '');
        },
      );
      ...
      final status = await upkeeper.check();
      check(status.state).equals(UpkeepState.outdated);
      check(status.summary).equals('1 tool(s) outdated (my_tool)');
      check(
        status.details,
      ).contains('  • my_tool (local): built earlier -> newer commit abcdef1');
    });
    ...
    test('update reports failure when dart install fails', () async {
      Directory('${tempInstallDir.path}/app-bundles/failing_pkg/hosted/1.0.0')
          .createSync(recursive: true);

      final upkeeper = DartInstallUpkeeper(
        installDirOverride: tempInstallDir,
        versionFetcher: (pkg) async => '2.0.0',
        processRunner: (executable, args) async {
          if (executable == 'dart' && args.contains('failing_pkg')) {
            return ProcessResult(1, 1, '', 'Compilation failed: syntax error');
          }
          return ProcessResult(0, 0, '', '');
        },
      );
      ...
      check(result.message).contains(
        'Failed to upgrade some Dart tools: failing_pkg: Compilation failed',
      );
      check(result.errorMessage)
          .isNotNull()
          .contains('failing_pkg: Compilation failed: syntax error');
    });
  });
}
